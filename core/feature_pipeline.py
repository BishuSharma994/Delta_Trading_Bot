...
def _load_events(event_type: str):
    """
    Load all events of a given type from JSONL.
    """
...
def build_feature_vector(symbol: str):
    ...
    # --- PRICE FEATURES ---
    prices_5m = [e["payload"]["mark_price"] for e in price_events[-5:]]
    prices_15m = [e["payload"]["mark_price"] for e in price_events[-15:]]
    ...
    features = {
        ...
        "time_to_funding_sec": None,  # filled later when clock sync added
    }
